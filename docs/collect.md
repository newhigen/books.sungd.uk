...
소장한 책만 넣는다. 아래는 서재 화면에 같이 뜨지만 산 책이 아니다.

- **샘플** — 킨들은 표지 위에 덧그린 배너(`overlay-banner__canvas`)로 가려낸다.
- **무료특별판** — 교보에서 제목에 `(무료특별판)` 이 붙는다. 샘플과 같은 것이라 뺀다.
- **시리즈 묶음** — 킨들의 `collection-title-*` 는 여러 권을 한 칸에 묶은 것이라, 안을 펼쳐
  개별 권으로 넣는 편이 낫다.
